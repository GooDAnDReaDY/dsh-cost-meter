...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function CostMeter(props) {
      ...
      // Rate row: label, cheap variant, expensive variant; active one filled.
      ...
      return h('div', { ref: box, style: { position: 'relative' } }, [
        ...
        open ? h('div', { key: 'panel', style: S.panel }, [
          h('div', { key: 'title', style: S.title },
            ...
            (flat ? 'единый тариф' : cheapNow ? 'не-пик' : 'пик')),

          h('div', { key: 'note', style: S.note },
            flat
              ? 'Ставка не зависит от времени суток'
              : (cheapNow ? 'Сейчас дешевле' : 'Сейчас полная ставка') +
                (left ? ' · смена через ' + left : '')),

          h('div', { key: 'clock', style: S.note },
            (windowText ? 'Скидка: ' + windowText + ' UTC · ' : '') + 'сейчас ' + hhmm(minute) + ' UTC'),

          h('div', { key: 'hr1', style: S.hr }),

          h('div', { key: 'rates', style: S.grid }, [
            h('div', { key: 'h0', style: S.th }, '1M токенов, $'),
            h('div', { key: 'h1', style: S.thNum }, flat ? 'ставка' : 'не-пик'),
            h('div', { key: 'h2', style: S.thNum }, flat ? '' : 'пик'),
