...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      'hint.useOpenRouter': 'Pull rates and discount windows automatically from the OpenRouter catalog.',
      'field.refreshHours': 'Catalog Refresh Interval (hours)',
      'hint.refreshHours': 'How often to fetch fresh model rates from OpenRouter (default: 24).',
    }
    ...
    const ru = {
      ...
      'field.useOpenRouter': 'Каталог OpenRouter',
      'hint.useOpenRouter': 'Автоматически загружать тарифы и скидочные окна из каталога OpenRouter.',
      'field.refreshHours': 'Интервал обновления каталога (часы)',
      'hint.refreshHours': 'Как часто обновлять ставки из OpenRouter (по умолчанию: 24).',
    }
    ...
    const cardCss = '.dcm-card { border:1px solid var(--dsw-alias-border-l2); background:var(--dsw-alias-bg-layer-3); border-radius:12px; list-style:none } ' +
      ...
      '.dcm-row-check { display:flex; align-items:center; gap:8px; padding:12px 0 } ' +
      '.dcm-check { width:16px; height:16px; accent-color:var(--dsw-alias-brand-primary, #6366f1); cursor:pointer } ' +
    ...
    function CostMeterCard(props) {
      ...
      const save = async () => {
        ...
        if (!Number.isFinite(hoursNum) || hoursNum <= 0) {
          broken.push('refreshHours: must be a positive number')
        } else {
          try {
          ...
          status === 'loading'
            ...
            : status !== 'ready'
              ...
              : React.createElement('div', null,
                  ...
                  React.createElement('span', { className: 'dcm-hint', style: { marginTop: -4, display: 'block', paddingBottom: 10 } }, t('hint.useOpenRouter')),
                  React.createElement('div', { className: 'dcm-field' },
