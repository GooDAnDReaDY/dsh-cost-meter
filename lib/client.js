...
`
    ...
    function CostMeter(props) {
      ...
      const settingsScope = getSettingsScope(ctx)
      const scope = React.useMemo(
        () => (settingsScope && typeof settingsScope.bind === 'function' ? settingsScope.bind({ namespace: NS }) : undefined),
        [settingsScope],
      )
      const settingsSnap = React.useSyncExternalStore
        ? React.useSyncExternalStore(
            ...
            React.useCallback(() => (scope && typeof scope.getSnapshot === 'function' ? scope.getSnapshot() : (settingsScope && typeof settingsScope.get === 'function' ? settingsScope.get(NS) : null)), [scope, settingsScope]),
