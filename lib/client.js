...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ru = {
      ...
      peak: 'пик',
      flat: 'единый',
      modelsBreakdown: t('modelsBreakdown'),
      sessionTokens: t('sessionTokens'),
      totalCost: t('totalCost'),
      noPriceSpecified: t('noPriceSpecified'),
      noUsageYet: t('noUsageYet'),
      routeUnknown: 'маршрут неизвестен',
      catalogUnavailable: 'Каталог OpenRouter недоступен: ',
      modelNotFound: t('modelNotFound'),
    ...
    function CostMeterCard(props) {
      ...
      const save = async () => {
        ...
        try {
          await scope.set('displayTimeZone', String(draft.displayTimeZone || 'Europe/Moscow').trim())
      ...
      setTimeout(() => {
        ...
        try {
          ctx.slots.inject('settings.section', () =>
            ctx.slots.register(
              { name: 'settings.section', id: '@goodandready/dsh-cost-meter', order: 25, locale: NS, label: () => makeT(useActiveLocale(ctx) === 'ru' ? ru : en, en)('title'), inject: () => ({ ctx }) },
              (p) => React.createElement(CostMeterCard, { ...p, ctx }),
            ),
          )
        } catch (_) {}
