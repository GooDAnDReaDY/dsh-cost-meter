...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const ru = {
      ...
      'badge.catalog_stale': 'Кэш: офлайн',
      'badge.deepseek': 'Тариф DeepSeek: активен',
      'section.general': 'Основные параметры',
      'section.general_desc': 'Отображение валюты, курса пересчёта, лимита бюджета и часового пояса',
      ...
      'field.budgetThreshold': 'Порог бюджета сессии (USD)',
      'hint.budgetThreshold': 'Оповещать цветом чипа при превышении суммы за сессию (0 — выключено).',
      ...
      'action.sync_now': 'Синхронизировать сейчас',
      'action.syncing': 'Синхронизация…',
      'action.sync_ok': 'Каталог обновлён',
      'action.sync_err': 'Ошибка синхронизации',
      'action.sync_rate_limited': 'Подождите {seconds} сек',
      'action.copy_report': 'Скопировать сводку',
      'action.copied': 'Скопировано!',
      'savings.hint': '💡 Скидка 50% начнётся через {time} ({clock} {zone})',
      'budget.exceeded': '⚠️ Превышен лимит сессии: {limit}',
...
.dcm-tip-box { padding: 8px 12px; border-radius: 8px; background: rgba(99,102,241,0.08); border: 1px solid rgba(99,102,241,0.2); color: var(--dsw-alias-label-primary); font-size: 12px; line-height: 1.4; display: flex; align-items: center; justify-content: space-between; gap: 8px; }
...
.dcm-chip-budget-warn { animation: dcm-pulse 2s infinite ease-in-out; }
@keyframes dcm-pulse { 0%, 100% { border-color: rgba(239,68,68,0.4); } 50% { border-color: rgba(239,68,68,0.9); } }
...
`
                    ...
                    React.createElement('div', { className: 'dcm-grid-2' },
                      ...
                      React.createElement('div', { className: 'dcm-field' },
                        ...
                        React.createElement('span', { className: 'dcm-hint' },
                          tzValid ? t('hint.displayTimeZone') : 'Некорректный идентификатор временной зоны IANA'
                        ),
                      ),
                    ),
                  ),
    ...
    /**
     * Fast O(K) lookup of next tariff rate boundary instead of a 1440-minute loop.
     */
    function minutesUntilChange(schedule, minute) {
      const windows = (schedule && schedule.windows) || []
      if (!windows.length) return null
      const now = ratesAt(schedule, minute)
      // Transition points can only occur at window start or window end
    ...
    /**
     * Memoized calculation of low and high rate extremes for a schedule.
     */
    ...
    function CostMeter(props) {
      ...
      const fg = budgetExceeded
        ? 'var(--dsw-alias-state-error-primary)'
        : (cheapNow ? 'var(--dsw-alias-state-success-primary)' : 'var(--dsw-alias-state-warning-primary)')
      const bg = budgetExceeded
        ? 'rgba(239,68,68,0.14)'
        : (cheapNow ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)')
      ...
      const copyReport = () => {
        const lines = [
          ...
          'Сессия: ' + (hasUsage ? money(total, currency) : '—') + (budgetThreshold > 0 ? ' (порог ' + money(budgetThreshold * fx, currency) + ')' : ''),
          'Тариф: ' + (cheapNow ? 'Льготный (не-пик)' : 'Полный (пик)'),
          'Ставки (1M): вход ' + rate(active.input) + ', выход ' + rate(active.output) + ', кэш ' + rate(active.cacheHit),
        ]
        if (hasUsage && usage) {
          lines.push('Токены: вход ' + tokens(usage.inputTokens) + ', выход ' + tokens(usage.outputTokens) + ', кэш-хит ' + tokens(usage.cacheReadTokens))
        }
      ...
      return h('div', { ref: box, style: { position: 'relative' } }, [
        h('button', {
          key: 'chip', type: 'button',
          className: 'dcm-chip' + (budgetExceeded ? ' dcm-chip-budget-warn' : ''),
          style: {
            background: bg,
            color: fg,
            borderColor: budgetExceeded
              ? 'rgba(239,68,68,0.5)'
              : (cheapNow ? 'rgba(16,185,129,0.3)' : 'rgba(245,158,11,0.3)'),
          },
          onClick: () => setOpen((v) => !v),
          title: budgetExceeded
            ? t('budget.exceeded', { limit: money(budgetThreshold * fx, currency) })
            : (flat ? 'Единый тариф' : cheapNow ? 'Льготный тариф' : 'Полный тариф'),
        }, [
