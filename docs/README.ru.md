...
### Визуальные улучшения и языковой стандарт (v0.8.2)
- **Нативный точечный индикатор**: В шапке используется аккуратная статусная точка (зелёная — скидка 50%, янтарная — пик, красная пульсирующая — превышен бюджет).
- **Строка экономии кэша**: Отображение сэкономленных средств за счёт кэша контекста (например: `Кэш сэкономил: ≈ $0.14 (-68%)`).
- **Компактный список моделей**: Аккордеон со сворачиванием списка при использовании 3+ моделей в сессии.
- **Стандарт локализации DSH**: Встроенные словари `en` и `zh`. Русский язык предоставляется через `dsh-russian-lang`.
